...
namespace fbgemm_xpu {
    ...
    void SplitEmbeddingBackwardFindLongSegments::operator()(const sycl::nd_item<1>& item) const {
        ...
        for (auto run_id = blockIdx_x * blockDim_x + threadIdx_x; run_id < num_runs; run_id += blockDim_x * gridDim_x) {
            if (sorted_linear_indices_run_lengths_[run_id] >= max_segment_length_per_warp_) {
                // A segment with length > max_segment_length_per_cta is handled by more than 1 thread block.
                const int num_ctas_for_run =
                    use_deterministic_algorithms_ ? 1 : div_round_up(sorted_linear_indices_run_lengths_[run_id], max_segment_length_per_cta_);

                const auto long_run_idx = xpuAtomicAdd(&num_long_run_ids_[0], num_ctas_for_run);
                ...
                if (num_ctas_for_run > 1) {
                    const auto really_long_run_idx = xpuAtomicAdd(&num_really_long_run_ids_[0], 1);
                    grad_accum_counter_[really_long_run_idx] = num_ctas_for_run;
                    for (int i = 0; i < num_ctas_for_run; ++i) {
                        long_run_id_to_really_long_run_ids_[long_run_idx + i] = really_long_run_idx;
                    }
                }
            }
        }
    ...
    void MarkRunStartsKernel<index_t>::operator()(const sycl::nd_item<1>& item) const {
            ...
            if (tid >= total_elements_) return;
            
            // Check if this is the start of a new run
            bool is_run_start = (tid == 0) || (sorted_input_[tid] != sorted_input_[tid - 1]);
    ...
    void CompactRunsKernel<index_t>::operator()(const sycl::nd_item<1>& item) const {
        ...
        if (run_starts_[tid] == 1) {
            ...
            const int32_t run_idx = run_positions_[tid];  // Position from prefix sum
    ...
    void LinearizeIndexKernel<index_t, info_acc_t, nobag, vbe>::operator()(const sycl::nd_item<1>& item) const{
        ...
        if (nobag) {
            ...
            for (auto i = lane_id; i < L_warp; i += kThreadGroupSize) {
                ...
                linear_indices_[indices_start_warp + i] = hash_offset_warp + idx;
            }
            }
        } else {
            ...
            for (int32_t i = lane_id; i < L_warp; i += kThreadGroupSize) {
                const index_t idx = indices_[indices_start_warp + i];
                reinterpret_cast<uint32_t*>(&infos_[0])[indices_start_warp + i] =
                    info_warp;
                linear_indices_[indices_start_warp + i] = hash_offset_warp + idx;
            }
            }
                ...
                if (!is_index_select) {
                    ...
                    TORCH_CHECK(false, "linearize_index_kernel kernel for bag operations not implemented yet in SYCL backend");
                        ...
                        queue.submit([&](sycl::handler& cgh) {
                            cgh.parallel_for<LinearizeIndexKernel<index_t, int64_t, true, false>>(
                                ...
                                LinearizeIndexKernel<index_t, int64_t, true, false>(
                                    ...
                                    info_B_num_bits,
                                    info_B_mask,
                                    (1u << (kDefaultInfoNumBits - info_B_num_bits)) - 1,
