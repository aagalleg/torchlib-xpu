...
namespace fbgemm_xpu {
    ...
    public:
        SplitEmbeddingBackwardFindLongSegments(
            ...
            at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> num_long_run_ids,
            at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> long_run_id_to_really_long_run_ids,
            at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> num_really_long_run_ids,
    ...
    private:
        ...
        const at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> sorted_linear_indices_run_lengths_;
        mutable at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> long_run_ids_;
        mutable at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> num_long_run_ids_;
        mutable at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> long_run_id_to_really_long_run_ids_;
        mutable at::PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits> num_really_long_run_ids_;
    ...
    class LinearizeIndexKernel {
        public:
            LinearizeIndexKernel(
                ...
                at::PackedTensorAccessor32<index_t, 1, RestrictPtrTraits> linear_indices,
        ...
        private:
            ...
            mutable at::PackedTensorAccessor32<index_t, 1, RestrictPtrTraits> linear_indices_;
    ...
    //   CUDA Generated File: gen_embedding_backward_split_unweighted_nobag_device_kernel.cuh
    //   CUDA Generated Path: fbgemm_gpu/_skbuild/linux-x86_64-3.10/cmake-build/
    //   CUDA Lines: 33-90 (complete implementation)
    //
    ...
    //   Uses warp-level cooperation with sub-group shuffle operations for coalesced access.
    //   Processes no-bag embeddings where each index maps to a single output row.
    //   
    //   This helper is called within backward kernels to compute the sum of gradients
    //   for all occurrences of a specific embedding index.
    //
    ////////////////////////////////////////////////////////////////////////////////
        ...
        for (int32_t vec_start = 0;
            ...
            for (int32_t sl = sl_start; sl < sl_end; sl += kThreadGroupSize) {
                ...
                for (int32_t j = 0; j < kThreadGroupSize && sl + j < sl_end; ++j) {
                    ...
                    #pragma unroll kFixedMaxVecsPerThread
                    for (int32_t vec = 0; vec < kFixedMaxVecsPerThread && 
                         (((vec + vec_start) * kThreadGroupSize + threadIdx_x) * VEC_WIDTH) < D; 
                         ++vec) {
                            const int32_t d = 
                                (((vec + vec_start) * kThreadGroupSize + threadIdx_x) * VEC_WIDTH);
