"""
Test suite for jagged_index_select_2d_forward

Tests 2D index selection on jagged tensors with offset-based indexing.
"""
...
class TestJaggedIndexSelect(TestCase):
    ...
    def _run_test(self, num_sequences, num_cols, num_selected, device="xpu", 
        ...
        selected_lengths = []
        if num_selected > 0:
            # We need to get lengths on CPU to construct offsets easily or use GPU ops
        ...
        # 4. Run Operator
        output = ops.jagged_index_select_2d_forward(
    ...
    def test_values_dim_mismatch(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "values must be 2D tensor"):
             ops.jagged_index_select_2d_forward(
