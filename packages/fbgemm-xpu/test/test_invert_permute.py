...
class TestInvertPermute(TestCase):
    ...
    def _test_correctness(self, device, dtype):
        ...
        result_empty = fbgemm.ops.invert_permute(permute_empty)
        ...
        result_single = fbgemm.ops.invert_permute(permute_single)
        ...
        result_small = fbgemm.ops.invert_permute(permute_small)
        ...
        result_identity = fbgemm.ops.invert_permute(permute_identity)
        ...
        result_reverse = fbgemm.ops.invert_permute(permute_reverse)
        ...
        result_medium = fbgemm.ops.invert_permute(permute_medium)
        ...
        result_large = fbgemm.ops.invert_permute(permute_large)
    ...
    def test_invalid_input_2d(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "must be 1-dimensional"):
            fbgemm.ops.invert_permute(permute_2d)
    ...
    def test_invalid_dtype_float(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "must be int32 or int64"):
            fbgemm.ops.invert_permute(permute_float)
    ...
    def test_meta_function(self):
        ...
        with FakeTensorMode():
            ...
            fake_result = fbgemm.ops.invert_permute(fake_permute)
    ...
    def test_varying_sizes_xpu(self):
        ...
        for n in test_sizes:
            with self.subTest(size=n):
                ...
                result = fbgemm.ops.invert_permute(permute)
...
class TestCPUXPUParity(TestCase):
    ...
    def test_parity_small(self):
        ...
        for n in [10, 100, 1000]:
            with self.subTest(size=n):
                ...
                result_cpu = fbgemm.ops.invert_permute(permute_cpu)
                result_xpu = fbgemm.ops.invert_permute(permute_xpu)
    ...
    def test_parity_edge_cases(self):
        ...
        result_cpu_empty = fbgemm.ops.invert_permute(permute_empty)
        result_xpu_empty = fbgemm.ops.invert_permute(permute_empty.to("xpu"))
        torch.testing.assert_close(result_cpu_empty, result_xpu_empty.cpu())
        
        # Single element
        permute_single = torch.tensor([0], dtype=torch.int32)
        result_cpu_single = fbgemm.ops.invert_permute(permute_single)
        result_xpu_single = fbgemm.ops.invert_permute(permute_single.to("xpu"))
        torch.testing.assert_close(result_cpu_single, result_xpu_single.cpu())
        
        # Identity
        permute_identity = torch.arange(100, dtype=torch.int32)
        result_cpu_identity = fbgemm.ops.invert_permute(permute_identity)
        result_xpu_identity = fbgemm.ops.invert_permute(permute_identity.to("xpu"))
    ...
    def test_benchmark_xpu(self):
        ...
        for n in sizes:
            for dtype in [torch.int32, torch.int64]:
                ...
                # Warmup
                for _ in range(10):
                    _ = fbgemm.ops.invert_permute(permute)
