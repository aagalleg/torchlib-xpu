...
class TestSplitLookupBackwardUniqCacheLocations(unittest.TestCase):
    ...
    def test_uniq_cache_stochastic_rounding_enabled_smoke(self):
        """Smoke test for uniq-cache mode with stochastic rounding enabled."""
        inputs = _generate_uniq_cache_locations_data(
            T=4, B=16, D=64, num_rows_per_table=10, L=6,
            placements=[0, 2, 0, 2],
            output_dtype=SparseType.FP32,
        ...
        # Matches C++ ArgIndex_aux_bool enum in pt2_arg_utils.h
        inputs["aux_bool"][5] = True  # IDX_STOCHASTIC_ROUNDING
        ...
        before_momentum = inputs["momentum1"][0].clone()
        inputs_xpu = _move_to_device(copy.deepcopy(inputs), "xpu")
        grad_xpu = grad_output.to("xpu")
        self._run_forward_backward(inputs_xpu, grad_xpu)

        after_momentum = inputs_xpu["momentum1"][0].cpu()
        self.assertTrue(torch.isfinite(after_momentum).all())
        self.assertGreater(
            (after_momentum - before_momentum).abs().sum().item(), 0.0
        )
